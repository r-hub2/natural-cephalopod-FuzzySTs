...
p.value.mean.Student <- function(data.fuzzified, type, H0, H1, sig, dist.type, i=1, j=1, theta= 1/3, thetas = 1, p=2, q=0.5, breakpoints=100)  {
  ...
  if ((length(is.na(H0)==FALSE) != 2*(breakpoints+1))) {
    ...
    {
      p_L = pt( t.U ,  H0.L  , df = n-1)
      p_U = pt( sort(t.L, decreasing = TRUE),  sort(H0.U), df = n-1)
      
    } 
    else if  ( type == 2 ) # Unilateral test - For H1 means approximately Bigger than ...
    {
      p_L = 1-pt( t.U ,  H0.L  , df = n-1)
      p_U = 1-pt( sort(t.L, decreasing = TRUE) ,  sort(H0.U) , df = n-1)
      
    } 
    else if  ( (type == 0) && (t[1,1] >= H0[1,2]) ) # Bilateral test - For H1 means approximately equal to ... and t_l >= m_r
    {
      p_L = 2*(1-pt( t.U ,  H0.L  , df = n-1))
      p_U = 2*(1-pt( sort(t.L, decreasing = TRUE) ,  sort(H0.U)  , df = n-1))
      
    } 
    else if  ( (type == 0) && (t[1,2] <= H0[1,1]) ) # Bilateral test - For H1 means approximately equal to ... and t_r <= m_l
    {
      p_L = 2* pt( t.U ,  H0.L  , df = n-1)
      p_U = 2* pt( sort(t.L, decreasing = TRUE) ,  sort(H0.U)  , df = n-1)
