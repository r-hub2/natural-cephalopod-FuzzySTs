...
p.value.mean.poisson <-  function(data.fuzzified, type, H0, H1, sig, dist.type, i=1, j=1, theta= 1/3, thetas = 1, p=2, q=0.5, breakpoints=100)  {
  
  if (is.trfuzzification(data.fuzzified) == TRUE){
    data.fuzzified <- tr.gfuzz(data.fuzzified, breakpoints = breakpoints)
  } else if (is.fuzzification(data.fuzzified)==FALSE){stop("Error in the fuzzification matrix")}
  ...
  if (unique(class(H0) %in% v) == TRUE){H0 <- alphacut(H0, alpha_L)} else if (is.numeric(H0) == TRUE && length(H0) == 1){
    ...
    H0 <- alphacut(H0, alpha_L)} else if (is.alphacuts(H0)==FALSE){stop("Problems with alpha-cuts of H0")}
  if ((length(is.na(H0)==FALSE) != 2*(breakpoints+1))) {
  ...
  {
    p_L = ppois(t.U ,  H0.L)
    p_U = ppois(sort(t.L, decreasing = TRUE),  sort(H0.U))
  ...
  {
    p_L = 1-ppois(t.U ,  H0.L )
    p_U = 1-ppois(sort(t.L, decreasing = TRUE),  sort(H0.U))
  ...
  {
    p_L = 2*(1-ppois(t.U ,  H0.L ))
    p_U = 2*(1-ppois(sort(t.L, decreasing = TRUE),  sort(H0.U)))
  ...
  {
    p_L = 2* ppois(t.U ,  H0.L )
    p_U = 2* ppois(sort(t.L, decreasing = TRUE),  sort(H0.U))
  ...
  p_T = cbind(p_L,p_U);
  plot(p_T,alpha, type='l', lwd=3, col=1, xlim=c(-0.04,1.02), ylab=expression(mu), main="Membership function of the fuzzy p-value and the significance level")
  #s1 = supp(sig)[1]
  #s2 = core(sig)[1]
  #s3 = core(sig)[1]
  #s4 = supp(sig)[2]
  lines(c(sig,sig), c(0,1), xlim=c(-0.04,1.02) , type='l', lty=3, lwd=2, col=2 )#lines( c(0,s1,s2,s3,s4,1), c(0,0,1,1,0,0), type='l', lty=3, lwd=2, col=2 )
